...
wet_season_data <- bind_rows(pm10_df_wet, ozone_df_wet, no2_df_wet, so2_df_wet)
...
dry_season_data <- bind_rows(pm10_df_dry, ozone_df_dry, no2_df_dry, so2_df_dry)
...
ggplot(wet_season_data, aes(x=County, y=Observations, fill=Pollutant)) +
  ...
  theme_minimal() +
  scale_fill_manual(values=c("PM10"="lightblue", "Ozone"="lightgreen", "NO2"="orange", "SO2"="purple")) +
