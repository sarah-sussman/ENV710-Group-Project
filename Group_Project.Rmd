...
# 3 separate regression models log transformed
lm_PM2.5_log <- lm(`LIFETIME PREVALENCE` ~ log_PM2.5 + log_Temp + 
                     log_Precip + log_Population, data = final_df_log)
summary(lm_PM2.5_log)


lm_PM10_log <- lm(`LIFETIME PREVALENCE` ~ log_PM10 + log_Temp + 
                     log_Precip + log_Population, data = final_df_log)
summary(lm_PM10_log)


lm_ozone_log <- lm(`LIFETIME PREVALENCE` ~ log_Ozone + log_Temp + 
                     log_Precip + log_Population, data = final_df_log)
summary(lm_ozone_log)
...
# Use predict to simulate model predctability

```{r}
# Use predict to simulate model predictability
final_df_log$predicted <- predict(lm_all_log)
ggplot(final_df_log, aes(x = LIFETIME PREVALENCE, y = predicted)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "darkgreen", linewidth = 1) +
  labs(
    x = "Observed Lifetime Prevalence of Asthma",
    y = "Predicted Lifetime Prevalence of Asthma"
  ) +
  theme_minimal()

rmse <- sqrt(mean((final_df_log$LIFETIME PREVALENCE - final_df_log$predicted)^2))
print(rmse)
```
