...
#no2_ca<-read_csv("data/no2_ca.csv")
#so2_ca<-read_csv("data/so2_ca.csv") 
...
###Temp and Precip data pull and exploration
...
## Merging final data into one DF
...
final_df <- all_pollutant_summary %>% 
...
pop_ca <- pop_ca[pop_ca$County %in% common_counties, ]

common_counties <- toupper(common_counties)
vehicles_ca <- vehicles_ca[vehicles_ca$County %in% common_counties, ]
...
final_df <- final_df %>% mutate(County = toupper(County))
pop_ca   <- pop_ca %>% mutate(County = toupper(County))
vehicles_ca <- vehicles_ca %>% mutate(County = toupper(County))
asthma_2021_2022 <- asthma_2021_2022 %>% mutate(County = toupper(County))


```

```{r}
# Merge pop_ca with final_df first, then merge vehicles_ca
final_df <- final_df %>% 
  inner_join(pop_ca, by = "County") %>%
  inner_join(vehicles_ca, by = "County") %>%
  inner_join(asthma_2021_2022, by = 'County')
...
The results indicate that all Pearson correlation coefficients (r-values) are below 0.8 EXCEPT population and vehicles.
...
numeric_vars <- c("Ozone", "PM2.5", "PM10", "Century_Temp", 
                  "Century_Precip", "Population", "Total Vehicles")
...
```{r}
# Only air quality variables
model_pollutants <- lm(`LIFETIME PREVALENCE` ~ Ozone + `PM2.5` + PM10, data = final_df)

# Full model using all predictors
model_pop <- lm(`LIFETIME PREVALENCE` ~ Ozone + `PM2.5` + PM10 + Century_Temp +
                   Century_Precip + Population, data = final_df)

# Full model using all predictors
model_veh <- lm(`LIFETIME PREVALENCE` ~ Ozone + `PM2.5` + PM10 + Century_Temp +
                   Century_Precip + `Total Vehicles`, data = final_df)

summary(model_pollutants)
summary(model_pop)
summary(model_veh)
```
